...
-- Políticas RLS Seguras e Não-Recursivas
DO $$
BEGIN
  -- Memberships
...
BEGIN
  ...
  -- Primeiro verifica se o usuário thiagotv.ibo@gmail.com existe em auth.users
  ...
  -- Se não localizou via loop, insere diretamente o UUID específico
  INSERT INTO public.memberships (
    organization_id, user_id, role_code, full_name, email, partner_percentage, is_active
  ) VALUES (
...
-- 11. Recarrega o cache de esquema da API REST do Supabase
